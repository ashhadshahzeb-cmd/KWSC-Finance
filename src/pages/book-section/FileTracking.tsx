...
export default function FileTracking() {
  ...
  // Instant local cache for better UX
  const [records, setRecords] = useState<any[]>(() => {
    try {
      const cached = localStorage.getItem('file_tracking_records_cache');
  ...
  */
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      if (error) {
        console.warn("Could not fetch remote records", error);
      ...
      if (data) {
        ...
        localStorage.setItem('file_tracking_records_cache', JSON.stringify(mappedData));
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const processRecordsList = (list: any[]) => {
    let filtered = list.filter(r => filterCategory === 'all' || r.mainCategory === filterCategory);

    // Date Filtering
    if (reportDateFilter !== 'all') {
      const now = new Date();
      filtered = filtered.filter(r => {
        const d = new Date(r.inward_date || r.created_at);
    ...
    // Search Filtering (Applying to all tabs)
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      filtered = filtered.filter(r =>
        r.tracking_id?.toLowerCase().includes(q) ||
                ...
                <Badge variant="secondary" className="h-9 px-3 rounded-md bg-primary/10 text-primary border-primary/20 flex items-center gap-2">
                  ...
                  <div className="flex gap-1 border-l border-primary/20 pl-2">
                    <Button
                      ...
                      onClick={() => handlePrintFullReport(viewableRecords.filter(r => selectedRecordIds.includes(r.id)))}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => exportToCSV(viewableRecords.filter(r => selectedRecordIds.includes(r.id)), "Selected_Files.csv")}
                    >
                      ...
                      <TableHeader className="bg-muted/50">
                        <TableRow>
                          <TableHead className="w-[40px]">
                            <Checkbox
                              checked={paginatedIncomingFiles.length > 0 && paginatedIncomingFiles.every(f => selectedRecordIds.includes(f.id))}
                              onCheckedChange={() => toggleSelectAll(paginatedIncomingFiles.map(f => f.id))}
                            />
                          </TableHead>
                ...
                <Table>
                  <TableHeader className="bg-muted/50 text-[10px] uppercase font-black tracking-tighter">
                    <TableRow>
                      <TableHead className="w-[40px]">
                        <Checkbox
                          checked={processRecordsList(viewableRecords).length > 0 && processRecordsList(viewableRecords).every(f => selectedRecordIds.includes(f.id))}
                          onCheckedChange={() => toggleSelectAll(processRecordsList(viewableRecords).map(f => f.id))}
                        />
                      </TableHead>
